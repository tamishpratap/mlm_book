...
@section('title', 'Member Wallet Address')
@section('page-subtitle', 'View and replace the Web3 USDT (BEP-20) payout wallet address for any registered member.')
...
<div class="row">
    ...
    <div class="col-lg-5 mb-4">
        <x-admin.card title="Find Member">
            <form action="{{ route('admin.members.wallet-address') }}" method="GET" class="mb-3">
            ...
            <div class="list-group list-group-flush border rounded-3 overflow-auto" style="max-height: 400px;">
                @forelse($searchResults as $m)
                    <a href="{{ route('admin.members.wallet-address', ['member_id' => $m->id, 'q' => $searchQuery]) }}" 
    ...
    <!-- Right Column: Selected Member & Wallet Address Form -->
    <div class="col-lg-7">
        @if(session('success'))
        ...
        @if(isset($selectedMember))
            ...
            <x-admin.card title="Current Authoritative Wallet Address" class="mb-4">
                @if($selectedMember->wallet_address)
                    <div class="p-3 bg-light border rounded-3 d-flex align-items-center justify-content-between flex-wrap gap-2">
                        <div>
                            <span class="text-muted small d-block mb-1">USDT (BEP-20) BNB Smart Chain:</span>
                            <code class="fw-bold text-dark" style="font-size: 13px;">{{ $selectedMember->wallet_address }}</code>
                        </div>
                        <a href="https://bscscan.com/address/{{ $selectedMember->wallet_address }}" target="_blank" class="btn btn-sm btn-outline-primary">
                            <i class="fa fa-external-link me-1"></i> BscScan
                        </a>
                    </div>
                @else
                    <div class="alert alert-warning border mb-0 small">
                        <i class="fa fa-exclamation-circle me-1"></i>
                        <strong>No Wallet Address Configured:</strong> This member has not yet set up a Web3 USDT (BEP-20) wallet address. You can set one below.
                    </div>
                @endif
            </x-admin.card>

            <!-- Replace Wallet Address Form -->
            <x-admin.card title="Replace Wallet Address">
                <div class="alert alert-light border small text-muted mb-4">
                    <i class="fa fa-info-circle text-primary me-1"></i>
                    <strong>Storage Rule:</strong> This form directly updates <code>members.wallet_address</code>. No second table or column is used. Member balances and transaction records are not affected.
                </div>

                <form action="{{ route('admin.members.wallet-address.update', $selectedMember) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="wallet_address" class="form-label small fw-bold">New Wallet Address <span class="text-danger">*</span></label>
                        <input type="text" name="wallet_address" id="wallet_address" class="form-control font-monospace {{ $errors->has('wallet_address') ? 'is-invalid' : '' }}" placeholder="0x..." required spellcheck="false">
                        <div class="form-text small">Must be a valid 42-character hexadecimal BSC (BEP-20) address starting with 0x.</div>
                    </div>

                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('admin.members.wallet-address') }}" class="btn btn-outline-secondary btn-sm">Cancel</a>
        ...
        @else
            <div class="card shadow-sm border-0 text-center py-5">
                <div class="card-body">
                    ...
                    <p class="text-muted small">Please search and select a member from the directory on the left to view or update their wallet address.</p>
                </div>
            </div>
        @endif
    </div>
