...
@section('title', 'Active Members Management')
@section('page-subtitle', 'Manage, inspect, and monitor active verified platform members.')
...
<x-admin.card title="Active Members Directory">
    <x-slot name="headerAction">
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('admin.members.export', ['format' => 'csv']) }}" class="btn btn-sm btn-outline-primary">
    ...
    <!-- Search & Filter Form -->
    <form action="{{ route('admin.members.index') }}" method="GET" class="row g-3 mb-4">
        <div class="col-lg-4 col-md-6">
            <x-admin.form.group label="Search Active Members" for="q">
                <x-admin.form.input name="q" :value="$search" placeholder="Search by ID, Name, Email, Phone..." icon="search" />
            </x-admin.form.group>
        </div>

        <div class="col-lg-2 col-md-6">
            <x-admin.form.group label="Activity Status" for="status">
                <x-admin.form.select name="status" :selected="$status" placeholder="All Activity Statuses" :options="['active' => 'Active (Last 7 Days)', 'inactive' => 'Inactive', 'verified' => 'Verified Only', 'unverified' => 'Unverified Only']" />
            </x-admin.form.group>
        </div>
        ...
        <div class="col-lg-2 col-md-6 d-flex align-items-end mb-3">
            <div class="d-flex gap-2 w-100">
                ...
                @if($search || $status || $country || $dateFrom || $dateTo)
                    <a href="{{ route('admin.members.index') }}" class="btn btn-outline-secondary" title="Reset Filters"><i class="fa fa-refresh"></i></a>
                @endif
            </div>
        </div>
    </form>
    ...
    <!-- Members Data Table -->
    <x-admin.table :headers="['', 'Avatar', 'Member ID', 'Name & Email', 'Phone', 'Location', 'Status', 'Joined Date', 'Actions']" :empty="$members->isEmpty()" emptyMessage="No active members match your search or filter criteria.">
            ...
            <tr>
                ...
                <td>
                    @if($member->mobile_verified_at)
                        <x-admin.badge variant="success" :light="true">VERIFIED MEMBER</x-admin.badge>
                    @else
                        <x-admin.badge variant="warning" :light="true">UNVERIFIED MEMBER</x-admin.badge>
                    @endif

                    @if($member->isOnline())
                        <span class="badge bg-success rounded-pill ms-1" style="font-size: 8px;">Online</span>
                    @endif
                </td>
                <td>
                ...
                <td>
                    <div class="d-flex align-items-center gap-1">
                        <a href="{{ route('admin.members.show', $member) }}" class="btn btn-sm btn-icon btn-outline-primary" title="View Profile">
                            <i data-feather="eye" style="width: 14px; height: 14px;"></i>
                        </a>

                        <!-- Block Member Action -->
                        <form action="{{ route('admin.members.block', $member) }}" method="POST" class="d-inline" onsubmit="return confirm('Block member {{ $member->name }}? They will not be able to log in.')">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-icon btn-outline-warning" title="Block Member">
                                <i data-feather="slash" style="width: 14px; height: 14px;"></i>
                            </button>
                        </form>

                        <!-- Delete Action -->
                        <form action="{{ route('admin.members.destroy', $member) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete member {{ $member->name }}?')">
                            @csrf
                            @method('DELETE')
