...
export function ExpiredStoriesPage() {
  ...
  const fetchExpiredStories = useCallback((page = 1) => {
    setLoading(true);
    setError(null);
    const params = {
      page,
    ...
    storiesApi
      ...
      .catch((err) => {
        setError(err.message || 'Failed to load expired stories.');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  useEffect(() => {
    fetchExpiredStories(pagination.page);
  }, [fetchExpiredStories, pagination.page]);
  ...
  const handleDelete = (story) => {
    confirmHelper.confirmDelete({
      itemName: `Expired Story #${story.id} by ${story.member?.name || story.author?.name || 'Member'}`,
      onAccept: async () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await downloadBlobFromResponse(blob, 'expired-stories-export.csv');
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitle="Archive of expired ephemeral stories past their 24-hour window."
        breadcrumbs={[
      ...
      <StoryFilterBar
        ...
        onRefresh={() => fetchExpiredStories(pagination.page)}
        loading={loading}
      />
      ...
      <StoryTable
        ...
        onRetry={() => fetchExpiredStories(1)}
        emptyTitle="No Expired Stories Found"
        emptyMessage="There are currently no expired stories in the platform archive."
      />
    </div>
  );
}
