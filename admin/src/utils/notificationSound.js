/**
 * Notification Sound Manager for Admin Portal
 *
 * Provides safe, throttled, professional audio notification playback
 * for genuinely new notifications arriving in the admin session.
 *
 * Features:
 * 1. Resolves canonical backend audio URL: ${VITE_BACKEND_URL}/sounds/notification.mp3
 ...
 * 5. Bounded known-ID cache to prevent memory leaks during long-running admin sessions
 */

const STORAGE_KEY = 'mlm_admin_notification_sound_enabled';
...
/**
 * Check if notification sounds are enabled in admin preferences.
 * Defaults to true.
 */
...
function getAudioInstance() {
  ...
  if (!audioInstance) {
    ...
    try {
      ...
      audioInstance.volume = 0.65; // Soft, professional volume
...
export function playNotificationSound() {
  ...
  try {
    ...
    if (playPromise !== undefined && typeof playPromise.catch === 'function') {
      playPromise.catch(() => {
        // Autoplay policy or decoding rejection handled gracefully without uncaught errors
      });
    }
    return true;
  } catch {
...
/**
 * Reset notification snapshot tracking (e.g. on logout or admin switch)
 * @param {string|number|null} [newAdminId=null]
 */
export function resetNotificationSoundState(newAdminId = null) {
  currentAdminId = newAdminId ? String(newAdminId) : null;
 ...
 * @param {string|number} [adminId] - Identifier of current authenticated admin
 * @returns {boolean} true if sound was triggered, false otherwise
 */
export function processNewNotifications(notifications, adminId) {
  if (!Array.isArray(notifications) || notifications.length === 0) return false;

  const effectiveAdminId = adminId ? String(adminId) : null;

  // If a different authenticated admin logged in, reset snapshot
  if (effectiveAdminId && currentAdminId && currentAdminId !== effectiveAdminId) {
    resetNotificationSoundState(effectiveAdminId);
  } else if (effectiveAdminId && !currentAdminId) {
    currentAdminId = effectiveAdminId;
  }
